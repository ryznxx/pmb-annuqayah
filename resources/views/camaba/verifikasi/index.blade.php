            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                    ...
                    @if($user->validity?->is_payment_valid)
                        <span class="px-3 py-1 bg-green-100 text-green-700 text-xs font-bold rounded-full flex items-center w-fit gap-1">
                            <span class="material-symbols-outlined text-sm">check_circle</span> Terverifikasi
                        </span>
                    @else
                        <span class="px-3 py-1 bg-amber-100 text-amber-700 text-xs font-bold rounded-full flex items-center w-fit gap-1">
                ...
                <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                    ...
                    @endphp
                    <span class="px-4 py-1 {{ $statusClass }} text-xs font-black rounded-full uppercase tracking-widest">
                ...
                <div class="p-8">
                    <div class="relative flex flex-col gap-8">
                        ...
                        <div class="relative flex items-center gap-6">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center z-10 {{ $user->document ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-400' }}">
                                <span class="material-symbols-outlined text-sm">done</span>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold {{ $user->document ? 'text-slate-800' : 'text-slate-400' }}">Pengisian Berkas</h4>
                        ...
                        {{-- Step 2 --}}
                        <div class="relative flex items-center gap-6">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center z-10 {{ $user->payment ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-400' }}">
                                <span class="material-symbols-outlined text-sm">done</span>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold {{ $user->payment ? 'text-slate-800' : 'text-slate-400' }}">Konfirmasi Pembayaran</h4>
                        ...
                        {{-- Step 3 --}}
                        <div class="relative flex items-center gap-6">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center z-10 {{ ($user->validity?->final_status === 'valid') ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400' }}">
                                <span class="material-symbols-outlined text-sm">local_activity</span>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold {{ ($user->validity?->final_status === 'valid') ? 'text-slate-800' : 'text-slate-400' }}">Cetak Kartu Ujian</h4>
                                @if($user->validity?->final_status === 'valid')
                                    <button class="mt-2 px-4 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded-lg hover:bg-indigo-700">Cetak Sekarang</button>
                                @else
                                    <p class="text-xs text-slate-500 italic">Tombol akan aktif setelah status pendaftaran "Valid".</p>
                                @endif
                            </div>
                        </div>
                    </div>
