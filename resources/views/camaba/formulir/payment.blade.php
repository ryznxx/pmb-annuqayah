<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-50 py-10 px-4">
        <div class="max-w-7xl mx-auto">
            ...
            @if(session('success'))
                <div class="mb-6 p-4 bg-green-100 border border-green-200 text-green-700 rounded-2xl flex items-center gap-2 font-bold">
            ...
            @if(!$activeWave && !$payment)
                <div class="bg-white rounded-3xl p-16 text-center border border-slate-200 shadow-sm">
                    ...
                    <p class="text-slate-500 mt-2 max-w-md mx-auto">Saat ini tidak ada gelombang pendaftaran yang aktif. Silakan hubungi admin atau cek kembali secara berkala.</p>
                    <a href="{{ route('pembuka') }}" class="mt-8 inline-block px-10 py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-all">Kembali ke Dashboard</a>
                </div>
            @else
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                            <div class="p-6 border-b border-slate-100 bg-slate-900 text-white flex justify-between items-center">
                                ...
                                @if($activeWave)
                                    <div class="bg-indigo-500 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter">Aktif</div>
                                @endif
                            </div>
                            ...
                            <div class="p-8">
                                ...
                                <div class="space-y-4">
                                    <p class="text-sm font-bold text-slate-800 uppercase tracking-widest">Pilihan Bank Transfer:</p>
                                    
                                    <div class="flex items-center justify-between p-5 bg-slate-50 rounded-2xl border border-slate-100">
                                        <div class="flex items-center gap-4">
                                            <div class="w-14 h-9 bg-white rounded border border-slate-200 flex items-center justify-center font-bold text-xs text-blue-600">BCA</div>
                                            <div>
                                                <p class="text-[10px] text-slate-400 uppercase font-bold">Nomor Rekening</p>
                                                <p class="font-mono font-bold text-slate-900 text-lg">8720 9918 221</p>
                                            </div>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-[10px] text-slate-400 uppercase font-bold">Atas Nama</p>
                                            <p class="font-bold text-slate-800 text-sm italic">Adm. Universitas Annuqayah</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-amber-50 border border-amber-200 rounded-3xl p-6 flex gap-4">
                            ...
                            <div class="text-sm text-amber-900">
                                ...
                                <p>Pastikan nominal transfer sesuai dengan tagihan di atas dan simpan struk/bukti transfer Anda.</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="p-8">
                                @if($isSuccess)
                                    <div class="text-center py-6">
                                        ...
                                        <p class="text-xs text-slate-500 mt-2">Pembayaran berhasil diverifikasi. Silakan cetak kartu ujian.</p>
                                    </div>
                                @elseif($isWaiting)
                                    <div class="text-center py-6">
                                        ...
                                        <p class="text-xs text-slate-500 mt-2">Mohon tunggu, admin sedang memvalidasi bukti transfer Anda.</p>
                                    </div>
                                @elseif($activeWave)
                                    @if($isRejected)
                                        <div class="mb-6 p-4 bg-red-50 border border-red-100 rounded-2xl">
                                            <p class="text-[10px] font-bold text-red-600 uppercase mb-1">Ditolak Karena:</p>
                                    ...
                                    <form action="{{ route('payment.store') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        <input type="hidden" name="registration_period_id" value="{{ $activeWave->id }}">

                                        <div class="mb-6">
                                            <label class="block text-xs font-bold text-slate-500 uppercase mb-3">Upload Bukti Transfer</label>
                                            <div class="relative group border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:bg-indigo-50 hover:border-indigo-300 transition-all cursor-pointer">
                                                <input type="file" name="proof_file" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                                                <span class="material-symbols-outlined text-slate-400 group-hover:text-indigo-500 text-3xl mb-1">cloud_upload</span>
                                                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">JPG/PNG (Maks 2MB)</p>
                                            </div>
                                        ...
                                        <div class="mb-6">
                                            ...
                                            <input type="text" name="account_name" required value="{{ old('account_name') }}" placeholder="Contoh: Ahmad Subagjo"
                                        ...
                                        <button type="submit" class="w-full py-4 bg-slate-900 text-white font-black rounded-2xl hover:bg-slate-800 shadow-xl transition-all flex items-center justify-center gap-2">
                                            Kirim Konfirmasi
